...
const createAlertRules = async () => {
  ...
  try {
    response = await utils.http({
      method: 'get',
      url: `${state.get('plugins.grafana-cloud.grafana.url')}/api/ruler/grafana/api/v1/rules`,
  ...
  for (var filename in rules) {
    if (rulesExisting[rules[filename].name]) {
      ...
      const url = `${state.get('plugins.grafana-cloud.grafana.url')}/api/ruler/grafana/api/v1/rules/GrafanaCloud/${rules[filename].name}`
      var response
      try {
        response = await utils.http({
          method: 'delete',
    ...
    const url = `${state.get('plugins.grafana-cloud.grafana.url')}/api/ruler/grafana/api/v1/rules/GrafanaCloud`
    var response
    try {
      response = await utils.http({
        method: 'post',
...
const createAlertContactPoints = async () => {
  ...
  const url = `${state.get('plugins.grafana-cloud.grafana.url')}/api/alertmanager/grafana/config/api/v1/alerts`
  var response
  try {
    response = await utils.http({
      method: 'post',
...
const createDashboard = async (filepath) => {
  ...
  // Import dashboard
  const url = `${state.get('plugins.grafana-cloud.grafana.url')}/api/dashboards/db`
  var response
  try {
    response = await utils.http({
      method: 'post',
...
const destroyDashboard = async (filepath) => {
  ...
  // Import dashboard
  const url = `${state.get('plugins.grafana-cloud.grafana.url')}/api/dashboards/uid/${uid}`
  var response
  try {
    response = await utils.http({
      method: 'delete',
 ...
 * createDashboard() for each file in ./apps/APP/plugins/grafana-cloud/dashboards/*.json.
 */
 ...
 * ./apps/APP/plugins/grafana-cloud/dashboards/*.json and destroy synthetic
 * monitoring checks.
 */
